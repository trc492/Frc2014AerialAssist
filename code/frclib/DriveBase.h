...
private:
        ...
        )
    {
        ...
        RegisterTask(MOD_NAME,
                     TASK_START_MODE | TASK_STOP_MODE | TASK_PRE_PERIODIC);
...
public:
        ...
        )
    {
        ...
        if (m_driveBaseFlags & DRIVEF_FOUR_MOTORS)
        {
            m_startPos[IDX_LEFT_FRONT] =
                m_motorPosition->GetMotorPosition(m_motors[IDX_LEFT_FRONT]);
            m_startPos[IDX_RIGHT_FRONT] =
                m_motorPosition->GetMotorPosition(m_motors[IDX_RIGHT_FRONT]);
        }
    ...
    {
        TLevel(INIT);
        TEnterMsg(("gyro=%p,leftMotor=%p,rightMotor=%p,yPid=%p,turnPid=%p,motorPostion=%p",
                   gyro, leftMotor, rightMotor, yPidCtrl, turnPidCtrl,
                   motorPosition));

        CommonInit();
        m_gyro = gyro;
        m_motors[IDX_LEFT_REAR] = leftMotor;
        m_motors[IDX_RIGHT_REAR] = rightMotor;
        m_yPidCtrl = yPidCtrl;
    ...
    {
        TLevel(INIT);
        TEnterMsg(("gyro=%p,lf=%p,lr=%p,rf=%p,rr=%p,xPid=%p,yPid=%p,turnPid=%p,motorPosition=%p",
                   gyro, leftFrontMotor, leftRearMotor, rightFrontMotor,
        ...
        )
    {
        ...
        {
            lfEnc =
                m_encPolarities[IDX_LEFT_FRONT]*
                (m_motorPosition->GetMotorPosition(m_motors[IDX_LEFT_FRONT]) -
                 m_startPos[IDX_LEFT_FRONT]);
            rfEnc =
                m_encPolarities[IDX_RIGHT_FRONT]*
                (m_motorPosition->GetMotorPosition(m_motors[IDX_RIGHT_FRONT]) -
                 m_startPos[IDX_RIGHT_FRONT]);
